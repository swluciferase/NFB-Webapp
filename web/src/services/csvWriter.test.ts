...
describe('generateCsv — event column mapping', () => {
  ...
  it('Option B: uses hardwareEventWallclock for Event Date when set', () => {
    const originWallclock = new Date('2026-05-01T08:00:00.000Z').getTime();
    const sample: RecordedSample = {
      ...sampleNoEvent(0.5),
      hardwareEvent: 5,
    ...
    // Verify it does NOT equal the fallback (startTime + 0.5 s)
    const fallbackDate = new Date(startTime.getTime() + 0.5 * 1000);
    const fallbackStr = [
    ...
    // Must equal the formatted originWallclock
